...
void clear_buffer() {
#ifdef _WIN32
    // Если в буфере уже есть символы, считываем их до полного опустошения
    while (_kbhit()) {
        int ch = getchar();
        // Можно добавить проверку: если ch не равен '\n', игнорировать или обрабатывать по необходимости
    }
#else
    // Для других платформ можно использовать неблокирующий режим ввода (это сложнее и зависит от системы)
    int ch;
    // Читаем все символы до конца строки (это может заблокировать, если буфер пуст)
    while ((ch = getchar()) != EOF && ch != '\n');
    // После этого, если вдруг в буфере накопились ещё символы (например, несколько нажатий Enter),
    // можно попробовать ещё один раз пройтись по буферу.
    while ((ch = getchar()) == '\n')
        ;  // Поглощаем лишние символы новой строки
#endif
}
